...
4. Push the included GitHub workflow. It refreshes the desk every Monday at 08:15 UTC. You can also run it immediately from the **Actions** tab.

The source list lives at the top of `supabase/functions/refresh-literary-desk/index.ts`. Each source is labelled with its region and editorial focus; failed feeds are skipped so one publication cannot take down the desk.
